  ...
  try {
    ...
    res
      ...
      .json({
        ...
        detail: error.message || "smtp_error",
      });
  }
});
...
app.post("/otp/verify", async (req, res) => {
  try {
    ...
    if (!result.ok) {
      return res
        .status(400)
        .json({ error: "Invalid or expired code", reason: result.reason });
    }
...
app.post("/auth/signup", async (req, res) => {
  try {
    const { fullName, cnic, email, username, password, postId, code } =
      req.body || {};
...
app.put("/applicant", authenticate, async (req, res) => {
  try {
    ...
    if (applicant.submissionStatus === "SUBMITTED") {
      return res
        .status(400)
        .json({ error: "Application already submitted, edits not allowed" });
    }
    ...
    res
      .status(500)
      .json({ error: "Unable to update applicant", detail: error.message });
  }
});
...
app.post("/application/submit", authenticate, async (req, res) => {
  try {
    ...
    const hasDocs =
      applicant.urlCv &&
      applicant.urlAcademicCerts &&
      applicant.urlExperienceCerts &&
      applicant.urlCnic;
    if (!hasProfile || !hasProfilePic || !hasQuals || !hasExps || !hasDocs) {
      return res
        .status(400)
        .json({ error: "Missing required data before submission" });
    }
    ...
    res
      .status(500)
      .json({ error: "Unable to submit application", detail: error.message });
  }
});
  ...
  try {
    ...
    const sectionTitle = (text) => {
      ...
      doc
        ...
        .font("Helvetica-Bold")
        .fontSize(12)
        .text(text, doc.page.margins.left, doc.y + 6, {
          width: pageWidth(),
          align: "center",
        });
      doc.restore();
    ...
    const subsectionTitle = (text) => {
      ...
      doc
        ...
        .font("Helvetica-Bold")
        .fontSize(11)
        .text(text, doc.page.margins.left + 6, doc.y + 3, {
          width: pageWidth() - 12,
        });
      doc.restore();
      doc.moveDown(0.6);
    ...
    const boxRow = (pairs) => {
      ...
      const labelWidth = 80;
      const lineHeight = 20;
      if (doc.y + lineHeight + 10 > doc.page.height - doc.page.margins.bottom)
        doc.addPage();
      const startY = doc.y;
      pairs.forEach((pair, idx) => {
        ...
        doc.save();
        doc
          .rect(x, startY, labelWidth, lineHeight)
          .fill("#f0f4ff")
          .stroke("#d0d7e4");
        doc
          .fillColor("#0f4ec7")
          .font("Helvetica-Bold")
          .fontSize(9)
          .text(label, x + 4, startY + 4, { width: labelWidth - 8 });
        doc.restore();

        doc.save();
        doc
          .rect(x + labelWidth, startY, colWidth - labelWidth, lineHeight)
          .fill("#fff")
          .stroke("#d0d7e4");
        doc
          .fillColor("#111")
          .font("Helvetica")
          .fontSize(9)
          .text(value || "-", x + labelWidth + 4, startY + 4, {
            width: colWidth - labelWidth - 8,
          });
        doc.restore();
      });
    ...
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text("Government of the Punjab", { align: "center" });
    doc
      .fontSize(14)
      .font("Helvetica-Bold")
      .text("Skills Development and Entrepreneurship Department", {
        align: "center",
      });
    doc.moveDown();
    doc
    ...
    boxRow([
      [
        "Date of Birth",
        applicant.dob ? applicant.dob.toISOString().slice(0, 10) : "-",
      ],
      ...
      applicant.qualifications.forEach((q, idx) => {
        ...
        boxRow([
          [
            "Degree Type",
            `${q.degreeType}${
              q.degreeType === "OTHER" && q.degreeTypeOther
                ? ` (${q.degreeTypeOther})`
                : ""
            }`,
          ],
          [
            "Field of Study",
            `${q.fieldOfStudy}${
              q.fieldOfStudy === "OTHER" && q.fieldOfStudyOther
                ? ` (${q.fieldOfStudyOther})`
                : ""
            }`,
          ],
        ...
        boxRow([
          ...
          [
            "Country",
            `${q.institutionCountry}${
              q.institutionCountry === "OTHER" && q.institutionCountryOther
                ? ` (${q.institutionCountryOther})`
                : ""
            }`,
          ],
        ]);
      ...
      applicant.experiences.forEach((e, idx) => {
        ...
        boxRow([
          [
            "Country",
            `${e.country}${
              e.country === "OTHER" && e.countryOther
                ? ` (${e.countryOther})`
                : ""
            }`,
          ],
    ...
    boxRow([
      [
        ...
        applicant.urlAcademicCerts ? "Uploaded" : "Missing",
      ],
      [
        "Experience Certificates",
        applicant.urlExperienceCerts ? "Uploaded" : "Missing",
      ],
    ]);
    boxRow([
    ...
    res
      .status(500)
      .json({ error: "Unable to generate PDF", detail: error.message });
  }
});
// Qualifications CRUD
  ...
  try {
    ...
    const created = await prisma.qualification.create({
      data: {
        ...
        fieldOfStudy,
        fieldOfStudyOther:
          fieldOfStudy === "OTHER" ? fieldOfStudyOther || "" : null,
        institutionName,
        institutionCountry,
        institutionCountryOther:
          institutionCountry === "OTHER" ? institutionCountryOther || "" : null,
    ...
    res
      .status(500)
      .json({ error: "Unable to create qualification", detail: error.message });
  }
});
...
app.put("/qualifications/:id", authenticate, async (req, res) => {
  try {
    ...
    const existing = await prisma.qualification.findFirst({
      where: { id, applicantId: req.user.sub },
    });
    if (!existing)
    ...
    res
      .status(500)
      .json({ error: "Unable to update qualification", detail: error.message });
  }
});
...
app.delete("/qualifications/:id", authenticate, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Unable to delete qualification", detail: error.message });
  }
});
...
app.post("/experiences", authenticate, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Unable to create experience", detail: error.message });
  }
});
...
app.put("/experiences/:id", authenticate, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Unable to update experience", detail: error.message });
  }
});
...
app.delete("/experiences/:id", authenticate, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Unable to delete experience", detail: error.message });
  }
});
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Unable to upload CV", detail: error.message });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          error: "Unable to upload academic documents",
          detail: error.message,
        });
    }
  }
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          error: "Unable to upload experience documents",
          detail: error.message,
        });
    }
  }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Unable to upload CNIC", detail: error.message });
    }
  }
);
